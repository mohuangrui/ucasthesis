...
Note: It is recommended to use Compile.bat to compile your files rather than to do compile in WinEdit. If you insist to compile in WinEdit, one choice is to move all the files into a single directory; the other choice is to provide the correct relative path for each file, for example, change \verb+\input{Frontpage}+ to \verb+\input{./Tex/Frontpage}+. 
