...
\begin{enumerate}
    ...
    \item 编译模板：
        \begin{enumerate}
            ...
            \item 任意系统：使用\LaTeX{}编辑器打开Thesis.tex文件进行编译亦可。
        \end{enumerate}
...
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.45\textwidth]{tc_q_criteria}
    \caption{Q判据等值面图。 \\ Fig.~\ref{fig:tc_q_criteria}: Isocontour of Q criteria.}
    \label{fig:tc_q_criteria}
\end{figure}
...
\end{verbatim}
            ...
            \item 封面下划线上的文本不居中下划线，这是因为那些下划线前面还有字头，导致文本只能在页面居中和在下划线上居中二选一。当前封面采取页面居中。如需要调整文本在下划线上的位置，可用 \verb|\hspace{+/- n.0em}| 命令来插入或删除 n 个空格，进行手动调整。这个解决方案是很不优雅，但问题的本质还是模板样式的不合理。有时下划线看上去粗细不一致，这是显示的问题，打印正常。
