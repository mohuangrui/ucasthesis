...
\begin{enumerate}
  ...
  \item 若不使用Compile.bat等编译脚本对本模板进行编译，则编译会出现无法找到相应文件的错误。 请在 "Thesis.tex" 和 "Main\textunderscore Content.tex" 中为所有文件提供完整的搜索路径。 如 \verb+\input{Appendix}+ 应改为 \verb+\input{./Tex/Appeddix}+。
\end{enumerate}
