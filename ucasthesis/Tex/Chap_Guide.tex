...
\begin{enumerate}
  ...
  “options --》 configuration wizard --》 wrapping” 
  ...
  “options --》 configuration wizard --》 Unicode” 
  
  中的“Enable ANSI Format...”选项。
  
  \item 若编译出现关于 “adobefonts” 的错误，则是由于系统缺乏 “adobe” 的相应字体，请在 “ucasthesis.cls” 文件中找到 “adobefonts”字符串，将其修改为 “winfonts”，但如此一来，可能出现拷贝pdf文档内容到word时存在乱码的问题，更好的解决方式是选择安装 “adobe reader” 软件和其相应的字体库。
\end{enumerate}

\section{Thesis.tex文档 }

Thesis.tex文档为主文档，其设计和规划了论文的整体框架，通过对其的阅读可以让用户了解整个论文框架的搭建。在本文作者写毕业论文的过程中，已进行了合适的设定，用户一般不需要再进行修改。

\section{Compile.bat and Clear\textunderscore Tmp.bat}

Compile.bat为编译此模板的Dos脚本，通过双击运行此脚本即可获得编译后的PDF文档（是的，前提是你的系统安装有\LaTeX{}编译器～），编译生成的文档及临时文件皆位于Tmp文件夹内。在此脚本中可以设定编译器为“pdflatex” or “xelatex” (default，且极力推荐使用xelatex 编译中文及中英文混排的\LaTeX{}文档)。Clear\textunderscore Tmp.bat脚本用于清空Tmp文件夹。
...
若编译过程出现关于“adobefonts”的错误，则是由于你的系统缺乏“adobe”的相应字体，请在“ucasthesis.cls”文件中找到 “adobefonts”字符串，将其修改为“winfonts”，但如此一来，可能出现拷贝pdf文档内容到word时存在乱码的问题，更好的解决方式是选择安装“adobe reader”软件和其相应的字体库。
...
\begin{itemize}
  ...
  \item Main\textunderscore Content.tex：文件中索引了将包含在论文中的所有Chapter内容，开始写论文时，可以只索引当前章节，以便快速编译和修改，当最终所有章节完成后，再全部索引即可。
...
Biblio文件夹内放置参考文献的索引信息文件：Myrefs.bib，参考文献的引用过程以实例的形式介绍，假设您需要引用名为“The use of fast Fourier transform for the estimation of power spectra: a method based on time averaging over short, modified periodograms”的文献，步骤如下：

1）使用“google scholar”搜索“The use of fast Fourier transform for the estimation of power spectra: a method based on time averaging over short, modified periodograms”，在目标条目下点击“引用”，展开后选择“ 导入BibTeX”，然后google将为你打开此文章的BibTeX索引信息，将它们copy添加到Myrefs.bib文件中。
...
撰写论文中，插图和制表常用到的命令，在Useful Commands.txt中给出了参考代码。
